...
public class TableMetadataMapper {
        ...
        public static PhysicalQuery mapToPhysicalQuery(
                ...
                Function<Long, TableMetadata> getTable = id -> tables.stream().filter(t -> t.getId().equals(id))
                                .findFirst()
                                .orElseThrow(() -> new ResourceNotFoundException("Table Metadata", id));

                BiFunction<Long, Long, ColumnMetadata> getColumn = (tableId, columnId) -> getTable.apply(tableId)
                                .getColumns()
