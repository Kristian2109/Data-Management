...
public class TableMetadataMapper {
     ...
     * @param tables the list of TableMetadata (should include all tables referenced
     *               in the query)
    ...
    public static PhysicalQuery mapToPhysicalQuery(
        ...
        Function<Long, TableMetadata> getTable = id -> tables.stream().filter(t -> t.getId().equals(id)).findFirst()
                .orElseThrow(() -> new ResourceNotFoundException("Table Metadata", id));

        BiFunction<Long, Long, ColumnMetadata> getColumn = (tableId, columnId) -> getTable.apply(tableId).getColumns()
                .stream().filter(c -> c.getId().equals(columnId))
                .findFirst()
        ...
        List<Select> physicalSelects = query.select().stream().flatMap(sel -> {
            ...
            return sel.columnIds().stream().map(colId -> {
                ...
                return new com.kris.data_management.physical.query.Select(col.getPhysicalName(),
